...
import { isKindTagKey, parseTagName, canonicalTagKey } from "@/lib/tags/tag-utils";
...
export function TagContextMenu({
  ...
  const showTags = Boolean(onSetNames);
  const showActions =
    Boolean(onNewFile) ||
    Boolean(onNewFolder) ||
    Boolean(onDownload) ||
  ...
  useEffect(() => {
    if (showTags) inputRef.current?.focus();
  ...
  return (
    ...
    >
      ...
      {showActions ? (
        <div className={showTags ? "border-t border-line py-1" : "py-1"}>
          {onNewFile ? (
            <button
              type="button"
              role="menuitem"
